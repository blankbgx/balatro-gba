...
当前自定义小丑已实装 **22 张（53-75）**：53 Wee、54 Riff-Raff、55 Baron、56 Mime、57 Egg、58 Smeared、59 Faceless、60 Gros Michel、61 Cavendish、62 Flower Pot、63 Loyalty Card、64 Riding the Bus、65 Ceremonial Dagger、66 Credit Card、67 Burglar、68 Flash Card、69 Showman、70 Card Sharp、**71 To the Moon（冲向月球，利息翻倍，被动不可复制，gfx0 slot 31）**、72 Splash（飞溅，上游美术 #69）、**73 Supernova（超新星，run 级牌型计数→倍率，gfx6 slot 4）、74 Green Joker（绿色小丑，出牌+1/弃牌-1，gfx2 slot 2）、75 Square Joker（方形小丑，4 张牌+4 筹码，gfx0 slot 32）**——73-75 卡面素材已全部就位（用户自绘，2026-08-20/21）。注册表已含全部已实现条目（0-75）；原版其余 joker（76+）尚未加入，需完整走：效果实现 → 精灵量化 → 注册 → 映射 → 测试。每个交付照常走：编译 → 命名时间戳 ROM → upload_rom.sh 上传。
...
- **卡包机制参考（2026-08-22，3DS 版源码验证）**：原版 Standard pack（标准包）选卡**直接进手牌且绕过 hand_size 上限**（手牌可临时超上限，如发 8 + 开包 2 = 10）；Arcana/Planet/Spectral 包当场使用不入手牌；Buffoon 包给小丑进 joker 栏。原版"0 手牌上限"靠 Standard pack 兜底可继续玩。**未来实装卡包时：Standard pack 加牌逻辑必须绕过 `get_effective_hand_size()` 检查**（勿顺手套上限）。另：3DS demake 的 `get_effective_hand_size_limit()` 就是 `math.max(1, ...)` clamp 1——本地 Stuntman 下限 1 与其一致。

- **⚠️ 手牌 0 判负 × pack 兜底交互（2026-08-31 用户视频考证）**：原版手牌上限 0 **不立即判负**，判负在**发牌时**（无法发牌立即判负），发牌前出售小丑可逆转。本地已对齐（M40：移除 clamp、`game_round_process_card_draw` 发牌时 `hand_size<=0 → LOSE`）。**但**：M40 的判负在发牌时立即触发，会**抢在原版"开 Standard pack 兜底"之前**（原版 0 上限靠开包继续玩）。**未来实装卡包时，此判负须重评估**——原版语义是"发牌失败但仍有包可开则暂缓判负"，需在发牌判负前给开包留窗口（或判负仅当无任何可开包时才触发）。

- **对局内出售小丑 = 原仓库故意行为（2026-08-22 用户查证）**：当前 `SELL_KEY` 守卫（e1d4902）只在商店态放行卖出——当时把对局内 L 键卖出当 bug 禁了，**实为误判**：原仓库故意支持对局内卖出，因为**部分 Boss 盲注要求对局内售出小丑解除 debuff**。当前 Boss 盲注全是占位（无实际效果），**暂不恢复**（避免误触隐患）；**待 Boss 盲注机制实装时（见盲注序列条目），须一并恢复对局内卖出**（去掉 `game_get_state() != GAME_STATE_SHOP` 限制或按 Boss 语义条件放行）。

- **Baseball Card 数值顺序待考证（2026-08-21 用户留记，M30）**：本地实现为「+N 全部先（INDEPENDENT 派发）→ ×1.5 逐个（动画队列，源分组）」；原版可能是「每张 Uncommon 计分时 +N 后紧跟全部 ×1.5ⁿ（广播）」。混合 +Mult 构筑下两者最终值可能不同。已实测本地行为 ✅，待对照原版视频/源码定夺后决定是否调整（改的话涉及 joker.c 钩子 + baseball_anim 队列的数值应用拆分）。

- **动画队列 × 状态机阻塞（2026-08-20 M29 教训，详见 gbalatro-joker-dev skill `references/animation-queue-state-gating.md`）**：任何阶段新增带动画的小丑效果时，必须考虑 ①多个同类效果同帧触发 → 串行队列；②动画播完前是否阻塞下游状态转换（按原版阶段语义判断）→ 在状态转换点加 `xxx_pending()` 门控；③数值立即生效、仅动画排队。派发陷阱：effect 返回 flag 时派发跨帧（一帧一个），返回 NONE 时同帧全遍历——把同步消息改为入队后必须补门控，否则动画与下游阶段并行。
