...
8. **空手牌状态机卡死（c67ab9d，用户实测）**：多张特技使手牌上限 ≤4 后，**打出/弃光全部手牌**（上限 >5 时永远留 3+ 张，此路径此前不可达）→ 游戏冻结不再进入计分。根因：HAND_PLAY→HAND_PLAYING 与 HAND_DISCARD→HAND_DRAW 两个过渡都在手牌循环体内（`i == 0` 门控），空手牌（`hand_top == -1`）时循环不执行 → 过渡永不触发。修复：循环后补空手牌过渡检查（同节拍门控；弃牌路径照常派发 ON_HAND_DISCARDED）

**复测结果**：✅ 修复后待验——多特技（≥2 张，上限 ≤4）打空手牌 → 应正常进入计分；弃光手牌 → 应正常重发
