...
**后续修正（同日，commit 紧随 M24）**：+3 / -N 标签覆写原打在数值**上方一行**，恰与面板背景图里烘焙的白色 hands/discards 标签重叠（白上白看不清）。改为**覆写数值自身区域**，颜色用该统计项的颜色（手数蓝/弃牌红），标签节拍结束后再原地滚动数字。复测：回合开始 +3/-4 短暂替换数值本身 → 原地滚动 → 上方白色 caption 始终干净。

### M23. 飞溅（Splash）结算后非牌型杂牌保持上浮不归位（P1）— 2026-08-18
...
**复测结果**：⏳ 待测（需飞溅构筑，DEBUG ROM 刷 reroll 获取）
