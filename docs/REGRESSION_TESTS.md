...
### M40. 手牌上限 0 不立即判负：判负时机改为发牌时（P1）— 2026-08-31, commit fcddf10

**功能**：对齐原版——手牌上限为 0（或负数，4+ 特技演员/多安迪）**不立即判负**；判负发生在**尝试发牌时**（无法发牌立即判负）。发牌前（商店）出售小丑恢复手牌上限即可逆转判负。

**背景**：M31 的 clamp≥1 是防御性修复（防 0 上限软卡死），但**偏离原版**（用户视频考证 2026-08-31）。

**实现**：
- `get_effective_hand_size()` **移除下限**（可返回 0/负——原版允许 0 上限）
- `game_round_process_card_draw()` 发牌前（特效 settle 后）加判负：`HAND_DRAW && hand_size <= 0 → game_change_state(GAME_STATE_LOSE)`
- 出售恢复上限天然成立：手牌上限是派生值，商店卖特技/安迪 → 上限恢复 → 发牌正常，判负不触发
- 判负点放在特效 settle beat 之后（原版顺序：盲注选择特效 → 发牌 → 判负）

**复测步骤**：
1. 4 特技演员（上限 0）→ 进对局 → **发牌时判负**（不再静默 clamp 发 1 张）
2. 4 特技演员 → 商店卖 1 张（上限 2）→ 进对局 → 正常发 2 张（逆转判负）
3. 5 特技（上限 -2）→ 判负（同 0）
4. 3 特技 + 2 安迪（8-6-2=0）→ 判负；卖 1 张 → 正常
5. 正常上限（无手牌小丑）→ 无回归

**复测结果**：⏳ 待 Delta 实测

**⚠️ 未来交互（pack 实装时）**：DEVELOPMENT_TODO 已记"原版 0 手牌上限靠 Standard pack 兜底可继续玩"——当前判负在发牌时立即触发，会**抢在开包兜底之前**。未来实装卡包时，此判负需延迟/重评估（仅当无包可开时才判负）。

---

### M39. Merry Andy 快乐安迪（84）实装：购入即 +3 弃牌 / -1 手牌上限（P1）— 2026-08-30, commit fcddf10

**功能**：Merry Andy 快乐安迪（orig #125，$7 Uncommon，Act N/A 全被动）——**+3 弃牌数 + -1 手牌上限**。
...
- 素材：gfx0 扩展 1120→1152px（slot 35），**8 色全部精确命中现有 palette**（零新增色）

**复测步骤**：
1. 商店买 Merry Andy（$7 Uncommon）→ **弃牌数立即 +3**（下回合 HUD 可见）
2. 手牌上限 8→7（发牌只发 7 张）
3. 安迪 + 窃贼同场 → 盲注选择后弃牌数 = 0（窃贼清零覆盖，安迪加成被吞——原版行为）
4. Showman 双安迪 → 弃牌 +6、手牌上限 6；卖出一张 → 恢复 +3/7
5. 安迪 + 多特技 → 手牌下限仍 1（不卡死）
6. 蓝图复制 → 无任何效果（不可复制）
7. 卡面：gfx0 slot 35 显示正常；商店画面无显示残留
...
3b. **roll 池限制**：目标永远是标准牌型；打出过 Flush Five/5oak/Flush House 后，这些才可能成为目标（用 Invisible/DNA 等造 5oak 验证）
...
**修复**：`count_to_the_moon()` 计数版（只认真身、跳过 expired——复制体 ID 不同天然不计，静默被动不可复制规则不变）；round_end.c 利息 = `层数 × (1 + count)`。3DS demake 同样是叠加语义（每实例独立加 `money/5`）。

**复测步骤**：
1. 1 张月球 + $25 → 利息 $10（5 层 × $2）
2. 2 张月球 + $25 → 利息 $15（5 层 × $3）
3. 0 张 → 利息 $5（回归检查）
4. 蓝图/脑暴复制 → 不计数（静默被动，不可复制）
5. 月球进入 expired 流程（灭绝/卖出瞬间）→ 当回合利息不计入

**复测结果**：⏳ 待 Delta 实测

---

### M35. Mr. Bones 骷髅先生（81）实装：≥25% 分数免死 + 救下回合只给利息（P1）— 2026-08-24, commit e36f9f7
...
- **复制语义**：反应式被动不可复制（无触发点；is_joker_owned 只认真身 ID）
- **特性2（NaN 分数不救）**：u32+clamp 下分数不可能 NaN——f64 分数改造落地时在 `mr_bones_can_save()` 加 `isnan(score)` 检查（代码注释已留标记）
- 素材：gfx18 扩展 64→96px（slot 2），**零新增色**（7 色全映射现有 palette），Ancient/Swashbuckler 槽零差异
...
6. boss 盲注救下 → ante 正常 +1，进入下一 ante
7. 两颗骷髅（Showman）→ 第一次免死消耗一颗，第二颗留给后续回合
8. 蓝图/脑暴复制 → 无效（不可复制）

**复测结果**：✅ 部分实测通过（2026-08-24）——救下后正确推进盲注选择（三个盲注流程）✅；满足最低分数时不消耗（不触发、不自毁）✅；**多骷髅实例：单回合拯救从左到右单个消耗（首颗自毁，其余保留）✅**；**蓝图/脑暴复制无效** ✅。**待验**：ante 8 boss 救下 → GAME_STATE_WIN 通关路径（需完整对局到 ante 8，暂未实测）

---

### M34. Gift Card 礼品卡（80）实装：回合结束所有小丑售价 +$1（P1）— 2026-08-24, commit 7000770

**功能**：Gift Card 礼品卡（orig #79，$6 Uncommon，Act N/A 回合结束事件）——**回合结束时所有持有小丑（含自身）售价 +$1**（原版含消耗牌，本地消耗牌未实装，仅小丑）。

**实现**：
- ON_ROUND_END：遍历 owned，每张 `joker->value += 2`（售价 = value/2 → +$1）
...
**复测结果**：✅ Delta 实测通过（2026-08-24）——本体与蓝图/脑暴复制均正常（用户确认，与 Egg/Gift Card 一并验证）
...
7. **下限保护（5b04976，用户实测修复前行为）**：修复前 4 张真身 → 上限 0 → 进入对局**软卡死不发牌**；5 张 → 上限 -2（int 有符号，非 unsigned 下溢）→ `card_draw` 的 `top >= size-1` 恒 true 提前返回 → **同样静默卡死不发牌**。修复后：4/5 张均 clamp 上限 **1**，发 1 张可正常推进。**⚠️ 2026-08-31 已改（M40）**：clamp 是偏离原版的临时 workaround——原版 0 上限不判负、发牌时才判负。clamp 已移除，0/负上限现在发牌时判负（见 M40）
8. **空手牌状态机卡死（c67ab9d，用户实测两条路径均卡死）**：多张特技使手牌上限 ≤4 后，**打出全部手牌**或**弃光全部手牌**（上限 >5 时永远留 3+ 张，此路径此前不可达）→ 游戏冻结不再进入计分/重发。根因：HAND_PLAY→HAND_PLAYING 与 HAND_DISCARD→HAND_DRAW 两个过渡都在手牌循环体内（`i == 0` 门控），空手牌（`hand_top == -1`）时循环不执行 → 过渡永不触发。修复：循环后补空手牌过渡检查（同节拍门控；弃牌路径照常派发 ON_HAND_DISCARDED）

**复测结果**：✅ Delta 实测通过（2026-08-24）——多特技最低手牌数 clamp 1 ✅；出牌打空手牌后正常进入计分 ✅；弃光手牌后正常重发 ✅（空手牌状态机卡死修复生效）
...
- **新 bug**：修复后在此文档**最上方**（"已修复 Bug 清单"顶部）插入一条，格式：`### M<N>. 标题（P0/P1/P2）— 日期, commit fcddf10`
