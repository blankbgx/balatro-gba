...
### M39. Merry Andy 快乐安迪（84）实装：购入即 +3 弃牌 / -1 手牌上限（P1）— 2026-08-30, commit 84b1b9c
...
- **售出行为（e871a37，用户关注）**：**对局内售出已被屏蔽**（joker_row.c guard `GAME_STATE_SHOP`——原版从不允许对局内售出），售出只在商店发生。商店售出安迪 → **-3 回退购时加成**（防买/卖循环刷弃牌）；手牌上限是派生值（发牌时实时读），售出后下回合发牌自然恢复，无残留。若未来开放对局内售出：手牌恢复后**不会立即补牌**，保持到下次发牌（原版行为）
