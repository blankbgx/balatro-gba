...
void main_bg_se_clear_rect(Rect se_rect)
{
    ...
    // Clears rows top..bottom-1 (bottom is EXCLUSIVE). Callers clearing a
    // 9-patch-filled area (which draws inclusively to bottom) must pass
    // bottom+1. Exclusive is kept because callers like round_end.c rely on
    // the row at `bottom` being untouched (POP_MENU popup source tiles).
